...
| Arithmetic | Enter `2 + 2`, then press `×` and continue | Operation line groups the completed immediate-evaluation segment as `( 2 + 2 ) ×`, then evaluates with full grouped expression context | `testMixedPrecedenceChainedPreviewUsesGroupedCompletedExpression` |
...
| Currency | Build `10 + 5 ×`, then type `$` before entering the next operand | Result display switches to currency (`$15`), but the operation line stays currency-free (`10 + 5 ×`) and continues evaluating correctly | `testTypingCurrencySymbolMidChainKeepsExistingExpression` |
