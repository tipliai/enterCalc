...
- `swift test list` discovers `128` `CalculatorViewModelTests` methods.
- This matrix documents the same `128` methods with no missing or extra entries.
...
| Currency | Paste `$12.3`, continue with `+ 1 =`, then clear all | Currency mode stays active for the pending operand and result (`$12.30 + $1 -> $13.30`) until all-clear resets the session | `testLeadingCurrencyPasteKeepsCurrencyActiveUntilAllClear` |
...
| Currency | Type `$`, then enter `1.2 + 2 =` | Leading currency symbol activates currency formatting immediately and arithmetic stays in currency mode (`$1.20 -> $3.20`) | `testTypingCurrencySymbolActivatesCurrencyFormatting` |
...
| Currency | Type `₿1.2` in currency mode | Bitcoin symbol is accepted as a supported currency prefix and copies back out as `₿1.20` | `testBitcoinCurrencySymbolIsSupported` |
...
| Currency | In French number style, paste `€1,2` and copy the result | Currency copy output respects the active locale decimal separator (`€1,20`) | `testCurrencyCopyUsesActiveNumberStyleDecimalSeparator` |
